...
class WorkerOrchestrator:
    ...
    def _handle_pull_request(
        ...
        # 3. Generate system prompt section from memory
        memory_prompt = context.to_system_prompt_section()

        # 4. Process findings (in a real deployment, this would call Gemini)
        # For now, we expose the pipeline structure and let findings be
        # injected or generated by the AI layer
        result = {
            "status": "processed",
            ...
            "active_decisions": len(context.active_decisions),
            "author_habits": len(context.author_habits),
        }

        logger.info(
            "PR #%d pipeline complete: memory loaded (%d decisions, %d habits)",
            pr_number, len(context.active_decisions), len(context.author_habits),
        )
